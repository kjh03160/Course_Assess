...
def crwaling(rq_year, rq_semester):
    ...
    button = driver.find_element_by_xpath('/html/body/div/form/div[1]/table/tbody/tr[5]/th/input')
    time.sleep(0.5)
    driver.execute_script("arguments[0].click();", button)

    lib_list = []
    lib_eles = driver.find_element_by_name('ag_compt_fld_cd')
    for i in lib_eles.find_elements_by_tag_name('option'):
    ...
    for k in range(len(lib_list)):
        ...
        for i in trs:
            ...
            try:
                Course(year=year, name=subject, syllabus=syllabus, prof=prof, credit=credit, dept=lib_list[k]).save()
